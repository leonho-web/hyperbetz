...
const Layout2Skeleton = () => (
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {[1, 2, 3].map(i => (
        <Skeleton key={i} className="w-full h-48 rounded-2xl bg-gradient-to-br from-muted/40 to-muted/20" />
      ))}
    </div>
  </div>
);
...
export const Layout2 = ({ slides, featuredGame, cards, isLoading = false }: Layout2Props) => {
  if (isLoading) {
    return <Layout2Skeleton />;
  }

  if (!slides?.length || !featuredGame || !cards?.length || cards.length < 3) {
