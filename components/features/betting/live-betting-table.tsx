...
function generateRealisticNickname(): string {
  const styles = [
    ...
    () => {
      ...
      return `***${prefix}${suffix}${number < 10 ? "0" + number : number}`;
    },
    ...
    // Style 5: VIP style
    () => {
      const levels = ["Bronze", "Silver", "Gold", "Platinum", "Diamond", "VIP"];
...
function generateBettingRecord(availableGames: Game[]): BettingRecord {
  ...
  if (multiplier === "FreeSpin") {
    ...
    if (freeSpinWin) {
      ...
      const winMultiplier =
        multiplierRange[Math.floor(Math.random() * multiplierRange.length)];
    ...
    if (multiplier === 0) {
      ...
      if (bigWinEvent.shouldTrigger && profit > 0) {
        profit = parseFloat((profit * (1 + Math.random() * 0.5)).toFixed(2)); // Up to 50% bonus
      }
    }
  }
...
export function LiveBettingTable({ className }: LiveBettingTableProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (burstChance < 0.15) recordsToAdd = 4; // 15% chance for 4 records
      else if (burstChance < 0.3) recordsToAdd = 3; // 15% chance for 3 records
          ...
          return (
            ...
            >
              <span className="flex items-center gap-1">
                {isSuperJackpot && (
                  <span className="text-purple-200 animate-bounce">💎</span>
                )}
                ...
                {profit.toFixed(2)}
                {isSuperJackpot && (
                  <span className="text-purple-200 animate-ping">✨</span>
                )}
                {isJackpot && !isSuperJackpot && (
                  <span className="text-red-200 animate-bounce">🔥</span>
                )}
  ...
  return (
    <div className={cn("w-full space-y-4", className)}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="bg-background/50 backdrop-blur-sm border border-border/50 rounded-lg p-3">
          <div className="text-xs text-muted-foreground">{t("biggestWin")}</div>
        ...
        <div className="bg-background/50 backdrop-blur-sm border border-border/50 rounded-lg p-3">
          <div className="text-xs text-muted-foreground">{t("bigWins50")}</div>
          <div className="text-lg font-bold text-orange-400">
            {stats.bigWinsToday}
          </div>
        </div>
        <div className="bg-background/50 backdrop-blur-sm border border-border/50 rounded-lg p-3">
          <div className="text-xs text-muted-foreground">{t("recentWins")}</div>
                      ...
                      <TableCell className="text-right">
                        <span
                          className={cn(
                            ...
                            record.bet >= 50 && "text-yellow-400 font-bold",
                            record.bet >= 200 && "text-orange-400 font-bold",
                            record.bet >= 500 &&
                        ...
                        >
                          ...
                          {record.bet < 1000 && record.bet.toFixed(2)}
                        </span>
                      </TableCell>

                      <TableCell className="text-center">
                        {getMultiplierDisplay(record.multiplier, record.status)}
                      </TableCell>

                      <TableCell className="text-right">
                        {getStatusBadge(record.status, record.profit)}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
