...
export function EmojiGifPicker({
  ...
  // Emoji states
  const [emojiCategories, setEmojiCategories] = React.useState<EmojiCategory[]>(
    []
  );
  ...
  const [activeGifCategory, setActiveGifCategory] = React.useState("trending");

  // Services
  const gifService = React.useMemo(() => GifService.getInstance(), []);
  ...
  }, [searchQuery, activeTab, searchEmojis, loadGifs]);
  ...
  const handleEmojiClick = (emoji: string) => {
    ...
    setRecentEmojis((prev) => {
      ...
      try {
        localStorage.setItem("chat-recent-emojis", JSON.stringify(updated));
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      >
        <TabsList
          className="grid w-full grid-cols-2 bg-card border-b border-sidebar-border 
        ...
        <TabsContent value="emojis" className="flex-1 flex flex-col mt-0">
          {emojiLoading ? (
            ...
            <div className="flex-1 flex items-center justify-center text-center p-4">
              <div>
                <p className="text-sm text-destructive mb-2">{emojiError}</p>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={loadEmojis}
            ...
            <ScrollArea className="flex-1 h-64 max-h-64 overflow-y-auto casino-scrollbar">
              <div className="p-3 space-y-4">
                ...
                {emojiCategories.length === 0 && !emojiLoading && (
        ...
        {/* GIF Tab */}
        <TabsContent value="gifs" className="flex-1 flex flex-col mt-0">
          {/* GIF Categories */}
          {!searchQuery && (
              ...
              <div className="flex gap-1 overflow-x-auto">
                {gifCategories.map((category) => (
                  <Button
                    key={category.key}
                    variant={
                      activeGifCategory === category.key ? "default" : "ghost"
                    }
                    size="sm"
                    onClick={() => handleGifCategoryClick(category.key)}
                    className="flex-shrink-0 text-xs"
                  >
                    <category.icon className="h-3 w-3 mr-1" />
                    {t(`picker.gifCategories.${category.key}`)}
                  </Button>
                ))}
              </div>
            </div>
          ...
          {gifLoading ? (
            ...
            <div className="flex-1 flex items-center justify-center text-center p-4">
              <div>
                <p className="text-sm text-destructive mb-2">{gifError}</p>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => loadGifs(activeGifCategory)}
                  className="text-[hsl(var(--foreground))]"
                >
                  {t("picker.retry")}
                </Button>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {gifs.map((gif) => (
                    <button
                      ...
                      className="relative group rounded-lg overflow-hidden hover:ring-2 hover:ring-primary transition-all duration-200 hover:scale-[1.02]"
                    >
                      <img
